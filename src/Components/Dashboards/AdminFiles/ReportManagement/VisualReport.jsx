...
// --- UPDATED TECHNICAL SCHEMAS FOR OIL & GAS ASSETS ---
const INSPECTION_SCHEMAS = {
  ...
  "Centrifugal / Reciprocating Pump (PU)": [
    { sn: "3.4.1", component: "Pump Casing and Covers", condition: "Satisfactory", notes: "" },
    { sn: "3.4.2", component: "Suction and Discharge Nozzles", condition: "Satisfactory", notes: "" },
    { sn: "3.4.3", component: "Mechanical Seal / Packing Area", condition: "Satisfactory", notes: "" },
    { sn: "3.4.4", component: "Bearing Housing", condition: "Satisfactory", notes: "" },
    { sn: "3.4.5", component: "Coupling and Guard", condition: "Satisfactory", notes: "" },
    { sn: "3.4.6", component: "Baseplate and Foundation Bolts", condition: "Satisfactory", notes: "" },
    { sn: "3.4.7", component: "Lubrication System", condition: "Satisfactory", notes: "" },
    { sn: "3.4.8", component: "Vibration and Noise Condition", condition: "Satisfactory", notes: "" },
  ],
  Default: [
  ...
  const WebView = () => {
    ...
    const PageHeader = () => (
      <div className="grid grid-cols-[1fr_2fr_1fr] border-2 border-slate-900 mb-6 text-center items-center font-bold">
        ...
        <div className="border-l-2 border-slate-900 p-2 text-[10px] text-blue-800 font-black uppercase">
          INSPECTPRO™
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-100 py-10 overflow-y-auto no-scrollbar font-sans text-black">
        ...
        <div className="max-w-[850px] mx-auto bg-white border border-slate-300 p-20 shadow-2xl mb-10 min-h-[1100px] flex flex-col print:m-0 break-after-page" style={{ breakAfter: "page" }}>
           ...
           <div className="text-center flex-1">
              ...
              <h2 className="text-2xl font-bold mb-10 uppercase tracking-widest">Visual Testing (VT) Report</h2>
              ...
              <div className="space-y-4 text-left inline-block font-bold">
                <p className="text-sm uppercase">Report ID: <span className="font-normal">{reportData.general.reportNum}</span></p>
                <p className="text-sm uppercase">Asset Ref: <span className="font-normal">{reportData.general.tag}</span></p>
        ...
        <div className="max-w-[850px] mx-auto bg-white border border-slate-300 p-12 shadow-2xl mb-10 min-h-[1100px] flex flex-col print:m-0 break-after-page" style={{ breakAfter: "page" }}>
          ...
          <div className="grid grid-cols-2 gap-px bg-slate-900 border-2 border-slate-900 text-[11px] mb-10">
            <div className="bg-slate-50 p-3 font-bold uppercase text-black">Lighting Method</div><div className="bg-white p-3 text-black">{reportData.environmental.lighting}</div>
            <div className="bg-slate-50 p-3 font-bold uppercase text-black">Surface Preparation</div><div className="bg-white p-3 text-black">{reportData.environmental.surface}</div>
            <div className="bg-slate-50 p-3 font-bold uppercase text-black">Access Method</div><div className="bg-white p-3 text-black">{reportData.environmental.access}</div>
            <div className="bg-slate-50 p-3 font-bold uppercase text-black">Equipment Temp</div><div className="bg-white p-3 text-black">{reportData.environmental.temp} °C</div>
          </div>
          <PageFooter pageNum={2} />
        </div>

        {/* PAGE 3: TECHNICAL FINDINGS [cite: 23, 24, 28, 60] */}
        <div className="max-w-[850px] mx-auto bg-white border border-slate-300 p-12 shadow-2xl mb-10 min-h-[1100px] flex flex-col print:m-0 break-after-page" style={{ breakAfter: "page" }}>
          <PageHeader />
          ...
          <table className="w-full border-collapse border-2 border-slate-900 text-[11px] mb-8">
            <thead>
              <tr className="bg-slate-100 uppercase font-black border-b-2 border-slate-900 text-black">
                <th className="p-3 text-left border-r-2 border-slate-900 w-16">S/N</th>
                <th className="p-3 text-left border-r-2 border-slate-900 w-1/3">Area</th>
                <th className="p-3 text-left border-r-2 border-slate-900">Status</th>
            ...
            <tbody>
              {reportData.observations.map((obs) => (
                <tr key={obs.sn} className="border-b border-slate-900 text-black">
                  ...
                  <td className={`border-r-2 border-slate-900 p-3 font-bold uppercase ${obs.condition !== 'Satisfactory' ? 'text-red-600' : 'text-emerald-700'}`}>{obs.condition}</td>
                  <td className="p-2"><textarea readOnly rows={2} value={obs.notes || "Satisfactory."} className="w-full bg-transparent border-none text-[10px] italic resize-none outline-none overflow-hidden text-black leading-snug" /></td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex-1">
            {reportData.images && reportData.images.length > 0 ? (
              <div className="grid grid-cols-2 gap-6">
                {reportData.images.map((img, idx) => (
                  <div key={idx} className="space-y-2">
                    ...
                    <p className="text-[10px] font-bold text-center uppercase border-b border-slate-900 pb-1 text-black">
                      {img.caption || `Photo ${idx + 1}: Observation Detail`}
                    </p>
                  </div>
                ))}
              </div>
              ...
              <div className="h-64 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center text-slate-400">
                ...
                <p className="text-xs uppercase font-bold tracking-tighter">No Photographic Evidence Attached</p>
              </div>
            )}
          </div>
          <div className="mt-8 p-4 bg-slate-50 border border-slate-200 text-[9px] italic text-black">
            <strong>Note:</strong> All photographs are timestamped and linked to the digital inspection manifest.
          </div>
          ...
          <div className="max-w-6xl mx-auto">
            <header className="flex justify-between items-center mb-8 bg-slate-900/40 p-6 rounded-3xl border border-slate-800 backdrop-blur-md">
              <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold uppercase tracking-tighter flex items-center gap-2"><Eye className="text-orange-500" /> VT Manifest</h1>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setReportMode(true)} className="bg-slate-800 px-6 py-2 rounded-xl text-xs font-bold border border-slate-700 hover:bg-slate-700">Preview</button>
                <button onClick={handleSaveToFirebase} disabled={isSaving} className="bg-orange-600 px-6 py-2 rounded-xl text-xs font-bold uppercase shadow-lg shadow-orange-900/20">{isSaving ? "Syncing..." : "Finalize Report"}</button>
              </div>
            </header>

            <div className="flex gap-6 border-b border-slate-800 mb-8 overflow-x-auto">
              {["logistics", "findings"].map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`pb-4 text-[10px] font-bold uppercase tracking-[0.2em] transition-all whitespace-nowrap ${activeTab === tab ? "text-orange-500 border-b-2 border-orange-500" : "text-slate-500"}`}>
                  {tab}
                </button>
              ))}
            </div>
            ...
            <div className="bg-slate-900/40 p-8 rounded-[2.5rem] border border-slate-800 backdrop-blur-sm min-h-[450px]">
              {activeTab === "logistics" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in duration-500">
                  ...
                  <InputField label="Ambient Temp (°C)" value={reportData.environmental.temp} onChange={(v) => setReportData({...reportData, environmental: {...reportData.environmental, temp: v}})} />
                </div>
              )}
                  ...
                  {reportData.observations.map((item, idx) => (
                    <div key={item.sn} className="grid grid-cols-12 gap-4 bg-slate-950/50 p-4 rounded-2xl border border-slate-800 items-center hover:border-orange-500/30 transition-colors">
                      ...
                      <div className="col-span-3">
                        <select className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2 text-[10px] font-bold text-orange-500 outline-none" value={item.condition} onChange={(e) => { const newObs = [...reportData.observations]; newObs[idx].condition = e.target.value; setReportData({...reportData, observations: newObs}); }}>
                          <option>Satisfactory</option><option>Non-Conformity</option><option>Degraded</option>
                        </select>
                      </div>
